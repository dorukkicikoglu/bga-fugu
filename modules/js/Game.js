...
class ModalBoxHandler {
    constructor(game, targetElement, contentHTML, shouldNudge = false, emphasized = false, destroyAfter = false, loadingBarDuration = null, onExpire) {
        this.game = game;
...
class HandHandler {
    ...
    updateMobileCardSpacing() {
        ...
        const cards = Array.from(this.cardsContainer.querySelectorAll('.a-card:not(.cloned-card)'));
        this.applyMobileCardSpacing();
...
HandHandler.MOBILE_HAND_SPAN_PERCENT = 99; //cards should span 98%-100% of the container's width
HandHandler.FACEDOWN_TO_FACEUP_OVERLAP_RATIO = 1.8; //facedown cards can overlap more since they don't need to stay readable
...
class CenterHandler {
    ...
    highlightHandForCenterCard(centerCardDiv) {
    ...
    highlightCenterForHandCard(handCardDiv) {
        const cardLocation = Number(handCardDiv.getAttribute('data-location-in-hand'));
        const handContainer = this.game.myself.getHand().getHandContainer();
...
class BackgroundHandler {
    ...
    displayMaxBubbles() {
        ...
        while (squirtDelay <= 5000 + (initialBubbleCount * 1000)) {
            const slowing = (squirtDelay > 5000);
